 ...
 *
 * Return:
 *  0 if the key was successfully generated, 1 otherwise
 */
...
int encrypt_data(const char* data, const char* key, char *output,
    ...
    if (EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, binary_key, iv) != 1) {
...
int decrypt_data(const char *data, const char *key, char *output,
    ...
    if (EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, binary_key, iv) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        return 1;
    }
